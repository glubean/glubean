...
3. **SDK: Remove `testCase`/`testSuite`** — Legacy APIs still exist in `mod.ts`, `types.ts`, `mod_test.ts`, `data.ts`, `data_test.ts`, `README.md`. Must remove before 0.1.0 (see SDK_API_REVIEW.md §1.1)
4. **Rewrite `glubean-tests/api.test.ts`** — Still uses bare `fetch()` (6 instances). Must use `ctx.http` — this is the primary example users will copy (see SDK_API_REVIEW.md §3.5)
