...
~115 `console.log` occurrences across packages (mostly in CLI commands and runner harness; many in comments/examples).
Consider structured logging and making debug output opt-in.
...
- ~~Make API URLs configurable~~ — CLI already supports `--api-url` and `GLUBEAN_API_URL` env; default extracted to
  `cli/lib/constants.ts` and unified to `api.glubean.com`. Worker and MCP were already configurable.
