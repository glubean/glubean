...
export async function contextCommand(
  ...
  const outputPath = options.out
    ? resolve(options.out)
    : resolve(dir, ".glubean", "ai-context.md");
  ...
  if (openapiPath) {
    console.log(
      `${colors.dim}Loading OpenAPI spec: ${relative(dir, openapiPath)}${
        colors.reset
      }`
    );
    try {
      ...
      if (endpoints.length > 0) {
        ...
        const coveredEndpoints = endpoints.filter((ep) =>
          coveredKeys.has(`${ep.method} ${ep.path}`)
        );
  ...
  console.log(
    `\n${colors.green}Context written to: ${relative(dir, outputPath)}${
      colors.reset
    }`
  );
  console.log(
    `${colors.dim}Reference it in Cursor with: @${relative(dir, outputPath)}${
      colors.reset
    }\n`
  );
}
