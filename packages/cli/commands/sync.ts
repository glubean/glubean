...
import { join, basename, resolve, relative } from "@std/path";
import { Tar } from "@std/archive/tar";
import { Buffer } from "@std/io/buffer";
import { walk } from "@std/fs/walk";
import { scan, type BundleMetadata, type FileMeta } from "@glubean/scanner";
...
export async function syncCommand(options: SyncOptions = {}): Promise<void> {
  ...
  const version =
    options.version || new Date().toISOString().replace(/[:.]/g, "-");
  const apiUrl = (
    options.apiUrl ||
  ...
  const breakdown =
    dataFileCount > 0
      ? ` (${fileCount} test + ${dataFileCount} data/support)`
      : "";
  console.log(
