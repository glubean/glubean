...
import { gitRoot, gitShow, isGitRepo } from "../lib/git.ts";
import {
  diffEndpoints,
  extractEndpoints,
  findOpenApiSpec,
...
function outputDiff(
  ...
  if (diff.added.length > 0) {
    ...
    for (const ep of diff.added) {
      console.log(
        `    ${colors.green}+ ${ep.method} ${ep.path}${colors.reset}${ep.summary ? ` — ${ep.summary}` : ""}`,
      );
  ...
  if (diff.modified.length > 0) {
    ...
    for (const { endpoint, changes } of diff.modified) {
      ...
      for (const change of changes) {
        const color = change.startsWith("+") ? colors.green : change.startsWith("-") ? colors.red : colors.yellow;
  ...
  if (diff.removed.length > 0) {
    ...
    for (const ep of diff.removed) {
      console.log(
        `    ${colors.red}- ${ep.method} ${ep.path}${colors.reset}${ep.summary ? ` — ${ep.summary}` : ""}`,
      );
    }
    console.log();
  }
