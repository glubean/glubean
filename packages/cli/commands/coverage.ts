...
export async function coverageCommand(
  ...
  if (traces && traces.traces.length > 0) {
    ...
    source = traces
      ? "last run (no traces recorded)"
      : "tag-based (no trace data — run tests first)";
  }
...
function matchTraceToEndpoint(
  ...
  for (const ep of endpoints) {
    ...
    const escaped = ep.path.replace(/[.*+?^${}()|[\]\\]/g, (m) =>
      m === "{" || m === "}" ? m : `\\${m}`
    );
...
function computeTraceCoverage(
  ...
  return endpoints.map((ep) => {
    ...
    return {
      ...
      source:
        testIds && testIds.size > 0 ? ("trace" as const) : ("none" as const),
    };
  });
}
...
function outputCoverageReport(
  ...
  console.log(
    `  ${colors.bold}Endpoints:${colors.reset} ${total} total, ${colors.green}${
      covered.length
    } covered${colors.reset}, ${
  ...
  if (covered.length > 0) {
    ...
    for (const r of covered) {
      ...
      const tests =
        r.testIds.length > 0
          ? ` — ${r.testIds.length} test${
              r.testIds.length > 1 ? "s" : ""
            } (${r.testIds.join(", ")})`
          : "";
      console.log(
  ...
  if (uncovered.length > 0) {
    ...
    for (const r of uncovered) {
      const method = r.endpoint.method.padEnd(7);
      const summary = r.endpoint.summary
        ? ` ${colors.dim}— ${r.endpoint.summary}${colors.reset}`
        : "";
      console.log(
