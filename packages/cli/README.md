...
Most users start with the [VS Code extension](https://marketplace.visualstudio.com/items?itemName=glubean.glubean), which auto-installs the CLI. This document covers the CLI directly — useful for CI pipelines, scripting, and advanced workflows.
...
You've been writing and running tests with the VS Code extension. Now you want them to run automatically on every PR. Here's how.
...
**3. Store secrets in CI** — add `API_KEY` and other secrets as repository secrets in GitHub/GitLab. Create `.env.ci.secrets` dynamically in your workflow:
...
Use this in CI to build custom notifications — for example, post a Slack message when `summary.failed > 0`, or send an email with the list of failed test names:
...
Writes a `.junit.xml` file that GitHub Actions, GitLab CI, Jenkins, and most CI systems natively understand. The XML follows the standard `<testsuite>` / `<testcase>` schema:
