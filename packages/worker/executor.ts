...
import type { RuntimeContext, RunEvent, Id } from "./types.ts";
...
async function calculateChecksum(filePath: string): Promise<string> {
  ...
  try {
    ...
    const hasher = new Uint8Array(32); // SHA-256 produces 32 bytes
...
async function loadLocalSecrets(
  ...
  if (context.secretsPath) {
    const resolved = context.secretsPath.startsWith("/")
      ? context.secretsPath
      : join(bundleDir, context.secretsPath);
...
export async function executeBundle(
  ...
  try {
    ...
    const secrets =
      context.secrets && Object.keys(context.secrets).length > 0
        ? context.secrets
    ...
    const overallTimeoutMs =
      context.limits?.timeoutMs ?? config.executionTimeoutMs;
    ...
    const requestedConcurrency =
      context.limits?.requestedConcurrency ?? config.executionConcurrency;
    const maxConcurrency =
      context.limits?.maxConcurrency ?? requestedConcurrency;
