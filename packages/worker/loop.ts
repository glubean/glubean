  ...
  const loop = async () => {
    while (!stopped) {
      ...
      try {
        ...
        const systemInfo =
          heartbeatCount % SYSTEM_INFO_EVERY_N_HEARTBEATS === 0
...
function createWorkerRuntime(options: RunWorkerOptions): {
  ...
  const lifecycle: Required<WorkerLifecycleOptions> = {
    ...
    maxTasksPerWorker:
      options.lifecycle?.maxTasksPerWorker ?? Number.POSITIVE_INFINITY,
  };
    ...
    logger.info("Worker loop started", {
      ...
      taskMemoryLimitMb:
        config.taskMemoryLimitBytes > 0
          ? config.taskMemoryLimitBytes / 1024 / 1024
          : "unlimited",
    });
  ...
  async function executeTask(
    ...
    try {
      ...
      if (result.aborted || result.timedOut) {
        const failureClass: FailureClass = result.timedOut
          ? "timeout"
          : "infra_error";
        const message =
          result.error ??
