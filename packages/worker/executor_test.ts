...
import type { RuntimeContext, RunEvent } from "./types.ts";
...
export const simpleTest = test({ id: "test-1", name: "Simple Test" }, async (ctx) => {
...
`;
  ...
  try {
    ...
    assertEquals(result.success, false, "Expected success to be false when test fails");
