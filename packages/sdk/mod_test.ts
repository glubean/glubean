...
Deno.test(
  ...
  () => {
    ...
    assertEquals(registry[0].steps, [{ name: "login" }, { name: "check perms" }]);
    assertEquals(registry[0].tags, ["auth"]);
    assertEquals(registry[1].steps, [{ name: "login" }, { name: "check perms" }]);
  }
);
...
Deno.test("test.pick - selects one example by default and injects _pick", () => {
  clearRegistry();
...
Deno.test("test.pick - respects GLUBEAN_PICK env var for specific selection", () => {
  clearRegistry();
...
Deno.test("test.pick - falls back to random when GLUBEAN_PICK key not found", () => {
  clearRegistry();
...
Deno.test("TestBuilder.use() - state flows through transform", async () => {
  ...
  const state = await t.steps![0].fn(ctx, undefined);
  await t.steps![1].fn(ctx, state);
});
...
Deno.test("EachBuilder.group() - marks steps with group id", () => {
  ...
  const tests = test
    ...
    .group("init", (b) =>
      b.step("seed", async (_ctx) => ({ seeded: true }))
    )
    .step("verify", async (_ctx, _state) => {})
    .build();
