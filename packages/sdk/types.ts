...
export interface ConfigureOptions {
  ...
  /**
   * Pre-configure a GraphQL client bound to a specific endpoint.
   *
   * **⚠️ Experimental** — This API may change in future minor releases.
   *
   * - `endpoint`: A var key (string) whose runtime value becomes the GraphQL URL.
   * - `headers`: Header values can use `{{key}}` syntax to interpolate secrets.
   * - `throwOnGraphQLErrors`: If `true`, throws when response contains errors.
   *
   * The returned `graphql` client auto-traces every operation and injects the
   * operation name into traces for dashboard visibility.
   *
   * @example
   * ```ts
   ...
   * ```
   */
  graphql?: ConfigureGraphQLOptions;
}
...
/**
 * GraphQL configuration for `configure()`.
 *
 * **⚠️ Experimental** — This API may change in future minor releases.
 *
 * Binds a `GraphQLClient` to a specific endpoint with optional default headers.
 * The returned client auto-traces every operation and injects the operation name
 * into traces for dashboard visibility.
 *
 * @example
 * ```ts
 * const { graphql } = configure({
 ...
 * ```
 */
export interface ConfigureGraphQLOptions {
  /**
   * Var key whose runtime value is used as the GraphQL endpoint URL.
   * This is a var key name, not the URL itself.
   *
   * @example "graphql_url" → resolved to ctx.vars.require("graphql_url")
   */
  endpoint: string;
...
> {
  ...
  /**
   * Pre-configured GraphQL client bound to the endpoint specified in
   * `configure({ graphql: { endpoint } })`.
   *
   * **⚠️ Experimental** — This API may change in future minor releases.
   *
   * Only available when `graphql` is specified in `configure()` options.
   * Auto-traces every operation and injects the operation name into traces.
   *
   * @example
   * ```ts
   * const { graphql } = configure({
   ...
   *   const { data } = await graphql.query<{ users: User[] }>(`
   *     query ListUsers { users { id name } }
   *   `);
   *   ctx.expect(data?.users.length).toBeGreaterThan(0);
   * });
   * ```
   */
  graphql: import("./graphql.ts").GraphQLClient;
}
