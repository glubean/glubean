 ...
 * @internal
 */
function getRuntime(): GlubeanRuntime {
...
function buildLazyHttp(httpOptions: ConfigureHttpOptions): HttpClient {
  ...
  const cache = new WeakMap<GlubeanRuntime, HttpClient>();
  ...
  for (const method of HTTP_METHODS) {
    // deno-lint-ignore no-explicit-any
    proxy[method] = (url: string | URL | Request, options?: any) =>
      getClient()[method](url, options);
  }
...
// =============================================================================
// Lazy GraphQL client builder
// =============================================================================

/**
 * Build a lazy GraphQL client proxy.
 * On first method call, resolves the config and creates a bound GraphQL client.
 *
 * @internal
 */
function buildLazyGraphQL(gqlOptions: ConfigureGraphQLOptions): GraphQLClient {
  // Cache the resolved client per runtime identity to avoid re-creating on every call.
  const cache = new WeakMap<GlubeanRuntime, GraphQLClient>();
  ...
  function getClient(): GraphQLClient {
    ...
    // Resolve endpoint from vars
    const endpoint = requireVar(gqlOptions.endpoint);

    // Resolve header templates
    const resolvedHeaders: Record<string, string> = {};
    if (gqlOptions.headers) {
      for (const [name, template] of Object.entries(gqlOptions.headers)) {
        resolvedHeaders[name] = resolveTemplate(
          template,
          runtime.vars,
          runtime.secrets,
    ...
    client = createGraphQLClient(runtime.http, {
      endpoint,
      headers: resolvedHeaders,
      throwOnGraphQLErrors: gqlOptions.throwOnGraphQLErrors,
    });
    cache.set(runtime, client);
    return client;
  }

  // Create a proxy that delegates to the lazily-resolved client.
  return {
    query: (query, options) => getClient().query(query, options),
    mutate: (mutation, options) => getClient().mutate(mutation, options),
  };
}

/**
 * Build a no-op GraphQL client that throws when accessed.
 * Used when `configure()` is called without `graphql` options.
 *
 * @internal
 */
function buildNoopGraphQL(): GraphQLClient {
  const msg =
    "GraphQL client not configured. " +
    'Add a `graphql` option to configure() — e.g., configure({ graphql: { endpoint: "gql_url" } })';
  return {
    query: () => {
      throw new Error(msg);
    },
...
> {
  ...
  const http = options.http
    ? buildLazyHttp(options.http)
    : buildPassthroughHttp();

  const graphql = options.graphql
    ? buildLazyGraphQL(options.graphql)
    : buildNoopGraphQL();

  return { vars, secrets, http, graphql };
}
...
function buildPassthroughHttp(): HttpClient {
  ...
  for (const method of HTTP_METHODS) {
    // deno-lint-ignore no-explicit-any
    proxy[method] = (url: string | URL | Request, options?: any) =>
      getRuntime().http[method](url, options);
  }
