/**
 * GraphQL client for Glubean tests.
 *
 ...
 *   const { data, errors } = await graphql.query<{ user: { name: string } }>(`
 *     query GetUser($id: ID!) {
 *       user(id: $id) { name }
 *     }
 *   `, { variables: { id: "1" } });
 ...
 * `;
 *
 * const { data } = await graphql.query(GET_USER, { variables: { id: "1" } });
 * ```
 */
export function gql(
...
export function createGraphQLClient(
  ...
  async function execute<T>(
    ...
    const mergedHeaders: Record<string, string> = {
      ...
      // Inject operation name for the harness to pick up in traces
      "X-Glubean-Op": opName,
    };
    ...
    // Only include operationName in the body if it's a real name (not "anonymous")
    if (opName !== "anonymous") {
    ...
    const response = await http
      .post(endpoint, {
        ...
        // GraphQL always returns 200, don't throw on HTTP status
