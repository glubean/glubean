...
// Branch node shapes live with the predicate machinery (contract-flow-condition.ts);
// imported type-only here so the RuntimeFlowStep / ExtractedFlowStep unions stay the
// single source of truth. (Type-only → no runtime cycle.)
import type {
  RuntimeBranchStep,
  ExtractedBranchStep,
  ExtractedPredicate,
