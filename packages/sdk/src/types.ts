...
export interface Test<S = unknown> {
   ...
   * `$index` stability flag). Present ONLY on `test.each` / `test.pick` rows;
   * absent for plain `test(...)` definitions. Mirrors the registry's
   * {@link RegisteredTestMeta.each} on the RUNTIME test object the runner
   * executes, so the run path can carry it to Cloud derive's row axis (B3,
   * `run-evidence-identity-model.md` §7). See {@link EachRowMeta}.
   */
  each?: EachRowMeta;
}
...
/**
 * Data-driven row provenance — set by `test.each` / `test.pick` for each
 * generated per-row test.
 *
 * Cloud derive otherwise sees only the final interpolated `id` (e.g. `user-1`,
 * `case-0`) and cannot tell a reorder-STABLE data-driven id from a positional
 * `$index`-based one: reordering the table keeps `user-$userId` pinned to its
 * row but shifts every `$index` id. The SDK owns id interpolation, so it is the
 * source of truth for whether the template referenced the positional `$index`
 * placeholder — this carries that truth to derive (B3 assertion-identity row
 * axis, `run-evidence-identity-model.md` §7).
 */
export interface EachRowMeta {
  /** Raw id template before interpolation, e.g. `"user-$userId"` or `"case-$index"`. */
  idTemplate: string;
  /** 0-based position of this row within the (filtered) data table. */
  index: number;
  /**
   * Reorder-stable per-row key: `idTemplate` interpolated with DATA fields only,
   * leaving the positional `$index` placeholder literal. For a purely
   * data-driven template this equals `id` and is a stable cross-run key; for a
   * `$index`-based template it retains the literal `$index` token (and `stable`
   * is `false`), so derive falls back to `index` to disambiguate rows.
   */
  rowKey: string;
  /**
   * `true` when `idTemplate` does NOT reference `$index` — i.e. `rowKey` is a
   * reorder-stable cross-run identity. `false` flags a positional (unstable) id.
   */
  stable: boolean;
}
...
export interface RegisteredTestMeta {
  ...
  /**
   * Data-driven row provenance (`idTemplate` + reorder-stable `rowKey` +
   * `$index` stability flag). Present ONLY for tests generated by
   * `test.each` / `test.pick`; absent for plain `test(...)` declarations.
   * Lets Cloud derive resolve a stable cross-run row identity instead of only
   * the final interpolated `id` (B3 row axis, see {@link EachRowMeta}).
   * `each.index` always equals {@link rowIndex} (B2 M3 selector protocol)
