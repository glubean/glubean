...
function isTest(obj: unknown): obj is Test<unknown> {
  return (
    isRecord(obj) &&
    "meta" in obj &&
    "type" in obj &&
    ((obj as { type?: unknown }).type === "simple" ||
      (obj as { type?: unknown }).type === "steps") &&
    isRecord((obj as { meta?: unknown }).meta) &&
    typeof (obj as { meta: { id?: unknown } }).meta.id === "string"
  );
}
  ...
  for (const [exportName, value] of Object.entries(module)) {
    ...
    tests.push({
      ...
      tags: Array.isArray(value.meta.tags)
        ? value.meta.tags
        : typeof value.meta.tags === "string"
        ? [value.meta.tags]
        : undefined,
      only: Boolean(value.meta.only),
      skip: Boolean(value.meta.skip),
    });
  }
...
async function runLocalTestsFromFile(args: {
  ...
  const testDir = absolutePath.substring(0, absolutePath.lastIndexOf("/"));
  const projectRoot = await findProjectRoot(testDir);

  const envPath = args.envFile
    ? resolve(args.envFile)
    : resolve(projectRoot, ".env");
  const secretsPath = resolve(projectRoot, ".env.secrets");
  ...
  const runNext = async (): Promise<void> => {
    while (!stop) {
      ...
      for await (const event of executor.run(fileUrl, test.exportName, {
        vars,
        secrets,
      })) {
        switch (event.type) {
...
server.registerTool(
  ...
  {
    description:
      "Discover Glubean test exports from a file path and return their metadata.",
    inputSchema: {
      filePath: z
        .string()
...
server.registerTool(
  ...
  {
    description:
      "Run Glubean test exports from a file locally and return structured results for AI debugging/fixing.",
    inputSchema: {
...
server.registerTool(
  ...
  {
    description:
      "List Glubean test files in a directory (lightweight index, no file writes).",
    inputSchema: {
      dir: z
        .string()
        .optional()
...
server.registerTool(
  ...
  {
    description:
      "Generate metadata (equivalent to metadata.json) in-memory for AI use, without writing to disk.",
    inputSchema: {
      dir: z
        .string()
        .optional()
...
server.registerTool(
  ...
  {
    description:
      "Trigger a remote run via Glubean Open Platform API (POST /open/v1/runs).",
    inputSchema: {
...
server.registerTool(
  ...
  {
    description:
      "Get run status via Glubean Open Platform API (GET /open/v1/runs/:runId).",
    inputSchema: {
      apiUrl: z.string().describe("Base API URL, e.g. https://api.glubean.com"),
  ...
  async (input: { apiUrl: string; token: string; runId: string }) => {
    ...
    const url = `${apiUrl.replace(/\/$/, "")}/open/v1/runs/${encodeURIComponent(
      runId
    )}`;
    const json = await fetchJson(url, {
      method: "GET",
...
server.registerTool(
  ...
  {
    description:
      "Fetch a page of run events via Glubean Open Platform API (GET /open/v1/runs/:runId/events).",
    inputSchema: {
      apiUrl: z.string().describe("Base API URL, e.g. https://api.glubean.com"),
      token: z.string().describe("Project token with runs:read scope"),
      runId: z.string().describe("Run ID"),
