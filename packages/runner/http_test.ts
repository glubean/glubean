...
export const httpGet = test("http-get", async (ctx) => {
...
Deno.test("ctx.http.post - auto-traces POST with JSON body", async () => {
  ...
  try {
    const testFile = await createHttpTestFile(`
export const httpPost = test("http-post", async (ctx) => {
...
Deno.test("ctx.http.get().json() - convenience JSON parsing", async () => {
  ...
  try {
    const testFile = await createHttpTestFile(`
export const httpJson = test("http-json", async (ctx) => {
...
Deno.test("ctx.http - callable shorthand works", async () => {
  ...
  try {
    const testFile = await createHttpTestFile(`
export const httpCallable = test("http-callable", async (ctx) => {
...
Deno.test(
  ...
  async () => {
    ...
    try {
      const testFile = await createHttpTestFile(`
export const httpExtend = test("http-extend", async (ctx) => {
...
Deno.test("ctx.http - multiple requests produce multiple traces", async () => {
  ...
  try {
    const testFile = await createHttpTestFile(`
export const httpMulti = test("http-multi", async (ctx) => {
...
Deno.test("ctx.http.extend - leading slash in path is normalized", async () => {
  ...
  try {
    const testFile = await createHttpTestFile(`
export const httpSlash = test("http-slash", async (ctx) => {
...
Deno.test("ctx.http - empty searchParams does not add bare '?'", async () => {
  ...
  try {
    const testFile = await createHttpTestFile(`
export const httpEmptyParams = test("http-empty-params", async (ctx) => {
...
Deno.test("ctx.http - non-empty searchParams still work", async () => {
  ...
  try {
    const testFile = await createHttpTestFile(`
export const httpWithParams = test("http-with-params", async (ctx) => {
  const res = await ctx.http.get("${baseUrl}/health", {
...
Deno.test("ctx.http - emits summary with request/error totals", async () => {
  ...
  try {
    ...
    const testFile = await createHttpTestFile(`
export const httpSummary = test("http-summary", async (ctx) => {
  await ctx.http.get("${baseUrl}/users");
  await ctx.http.get("${baseUrl}/health");
...
Deno.test({
  ...
  fn: async () => {
    const testFile = await createHttpTestFile(`
export const noHttp = test("no-http", async (ctx) => {
...
`);
    ...
    assertEquals(
      summaries.length,
      0,
      "No summary should be emitted when no HTTP calls are made"
    );
...
Deno.test("ctx.http - summary with all successful requests", async () => {
  ...
  try {
    const testFile = await createHttpTestFile(`
export const httpAllOk = test("http-all-ok", async (ctx) => {
  await ctx.http.get("${baseUrl}/users");
  await ctx.http.get("${baseUrl}/health");
});
`);
