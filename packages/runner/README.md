...
Sandboxed test execution engine for Glubean. Runs tests in isolated Deno subprocesses with HTTP interception, structured event streaming, and debug support.
...
The harness runs inside the subprocess, imports the test file, executes tests, and communicates results back to the executor via structured JSON on stdout.
