...
This guide covers the assertion and validation APIs available in the Glubean SDK.
...
Jest/Vitest-style fluent API. **Soft-by-default**: failed assertions are recorded but do not throw, so all checks run
...
ctx.expect(body.age).toBeGreaterThanOrEqual(1);
ctx.expect(body.age).toBeLessThan(200);
ctx.expect(res.status).toBeLessThanOrEqual(299);
...
By default, `ctx.expect` is soft (records failure but continues). Use `.orFail()` when subsequent code depends on the
assertion passing:
...
In API testing, you often want to see **all** failures at once rather than fixing them one at a time. Soft assertions
collect every failure in a single run:
...
> **Tip:** Prefer `ctx.expect` for readability. Use `ctx.assert` when you need direct boolean logic or are migrating
> from the older API.
...
Warnings are recorded but **never** affect test pass/fail. Use them for best-practice checks, performance budgets, or
deprecation notices.
...
Validate data against a schema library (Zod, Valibot, ArkType, or any object with `safeParse` or `parse`). Returns the
...
`ctx.validate` works on individual values too — useful for checking dynamic fields like IDs, timestamps, or tokens
without writing regex by hand:

```ts
import { z } from "zod";

ctx.validate(body.id, z.string().uuid(), "user id");
ctx.validate(body.email, z.string().email(), "email format");
ctx.validate(body.createdAt, z.string().datetime(), "ISO timestamp");
ctx.validate(body.score, z.number().min(0).max(100), "score range");
ctx.validate(body.role, z.enum(["admin", "user", "guest"]), "valid role");
```

Zod ships with a rich set of built-in validators: [zod.dev](https://zod.dev) — `uuid`, `email`, `url`, `datetime`, `ip`,
`cuid`, `regex`, `min`, `max`, `int`, `positive`, `enum`, and many more.

> **Tip:** For quick one-off checks, `ctx.expect(val).toMatch(/pattern/)` is simpler. Reach for `ctx.validate` when you
> want structured error messages or reusable schemas.
...
The SDK uses a `SchemaLike<T>` protocol — any library that implements `safeParse()` or `parse()` works:
...
Validate request and response bodies automatically by adding a `schema` option to any `ctx.http` call.
